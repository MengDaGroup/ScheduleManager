package com.akee.versionmanager.service;
...
import com.akee.versionmanager.manager.DownloadManager;
import com.akee.versionmanager.manager.HttpDownloadManager;
...
import com.akee.versionmanager.utils.NotificationUtil;
...
public final class DownloadService extends Service implements OnDownloadListener {
    ...
    private static final String TAG = VersionConstant.TAG + "DownloadService";
    ...
    private void init() {
        ...
        //创建apk文件存储文件夹
        VersionFileUtil.createDirDirectory(downloadPath);
        ...
        if (checkApkMD5()) {
            ...
            //直接调用完成监听即可
            done(VersionFileUtil.createFile(downloadPath, apkName));
        } else {
    ...
    private boolean checkApkMD5() {
        if (VersionFileUtil.fileExists(downloadPath, apkName)) {
            String fileMD5 = VersionFileUtil.getFileMD5(VersionFileUtil.createFile(downloadPath, apkName));
    ...
    public void done(File apk) {
        ...
        if (showNotification || Build.VERSION.SDK_INT >= 29) {
            ...
            NotificationUtil.showDoneNotification(this, smallIcon, downloadCompleted,
                    clickHint, VersionConstant.AUTHORITIES, apk);
        }
        if (jumpInstallPage) {
            VersionApkUtil.installApk(this, VersionConstant.AUTHORITIES, apk);
        }

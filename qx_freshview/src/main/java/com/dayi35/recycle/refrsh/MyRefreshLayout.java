...
public class MyRefreshLayout extends RefreshInterceptLauyout implements OnHeaderListener {
    ...
    private TextView mTvHeaderRefreshStatus;
...
//        inflateHeaderView();
    ...
    private void inflateHeaderView() {
        View view = LayoutInflater.from(context).
                inflate(R.layout.rv_pull_refresh, null);
        mIvHeaderRefresh = view.findViewById(R.id.iv_refresh);
        mTvHeaderRefreshStatus = (TextView) view.findViewById(R.id.tv_status);
        mTvHeaderRefreshStatus.setTextColor(getResources().getColor(R.color.refresh_text_color));
        addHeader(view);
        setOnHeaderListener(this);
        mRefreshingAnimation = (RotateAnimation) AnimationUtils.loadAnimation(
                context, R.anim.common_refresh_rotating);
        // 添加匀速转动动画
...
//                    mListener.onLoadMore();
    ...
    public void onRefreshBefore(int scrollY, int refreshHeight, int headerHeight) {
        mTvHeaderRefreshStatus.setText(R.string.rv_refresh_down);
    }

    @Override
    public void onRefreshAfter(int scrollY, int refreshHeight, int headerHeight) {
        mTvHeaderRefreshStatus.setText(R.string.rv_refresh_release);
    }
    ...
    public void onRefreshing(int scrollY, int refreshHeight, int headerHeight) {
        mTvHeaderRefreshStatus.setText(R.string.rv_refresh_ing);
        mIvHeaderRefresh.startAnimation(mRefreshingAnimation);
        if (null != mListener){
            mListener.refresh();
        }
    }
    ...
    public void onRefreshComplete(int scrollY, int refreshHeight, int headerHeight, boolean isRefreshSuccess) {
        mTvHeaderRefreshStatus.setText(R.string.rv_refresh_success);
        mIvHeaderRefresh.clearAnimation();
    }

    @Override
    public void onRefreshCancel(int scrollY, int refreshHeight, int headerHeight) {
        mTvHeaderRefreshStatus.setText(getResources().getString(R.string.rv_refresh_cancel));
        mIvHeaderRefresh.clearAnimation();
    }

...
public abstract class RefreshInterceptLauyout extends RefreshBaseLayout {
    ...
    // 用于计算滑动距离的Y坐标中介
    public int mLastYMove;
    // 用于判断是否拦截触摸事件的Y坐标中介
    public int mLastYIntercept;

    //是否可以加载更多
    public boolean mIsCanLoad = true;
    //是否可以下拉刷新
    public boolean mIsCanRefresh = true;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (y > mLastYIntercept) { // 下滑操作
                    ...
                    if (!mIsCanRefresh) intercept = false;
    ...
    private View getLastVisiableChild() {
        for (int i = mLastChildIndex; i >= 0; i--) {

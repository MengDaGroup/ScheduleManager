...
public class QRCodeUtil {
     ...
     * @param context 上下文
     * @param content 内容
     * @param standard 标准（大小）
     * @return
     */
    public static Bitmap createQRCodeBitmap(Context context, String content, int standard) {
//        int height = DensityUtil.dp2px(context, context.getResources().getDimension(R.dimen.dp_60));
        int height = standard;
        int width = standard;
        // 字符串内容判空
        if (TextUtils.isEmpty(content)) {
        ...
        try {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    //bitMatrix.get(x,y)方法返回true是黑色色块，false是白色色块
                    if (bitMatrix.get(x, y)) {
                        pixels[y * width + x] = context.getResources().getColor(R.color.utils_color_qr_black);//黑色色块像素设置
                    } else {
                        pixels[y * width + x] = context.getResources().getColor(R.color.utils_color_qr_white);// 白色色块像素设置
                    }
                }
            }
            /** 4.创建Bitmap对象,根据像素数组设置Bitmap每个像素点的颜色值,并返回Bitmap对象 */
     ...
     * @param context 上下文
     * @param content 内容
     * @param logoBitmap log
     * @param standard 标准（大小）
     * @return
     */
    public static Bitmap createQRCodeBitmap(Context context,String content, Bitmap logoBitmap, int standard) {
//        int height = DensityUtil.dp2px(context, context.getResources().getDimension(R.dimen.dp_60));
        int height = standard;
        int width = standard;
        ...
        try {
            ...
            /** 5.为二维码添加logo图标 */
            if(logoBitmap != null){
                return addLogo(bitmap, logoBitmap, 0.3f);
            }
            return bitmap;
        } catch (WriterException e) {
            e.printStackTrace();

...
import com.dayi35.qx_widget.pickers.util.LogUtils;
import com.dayi35.qx_widget.pickers.widget.WheelListView;
...
public class AddressPicker extends LinkagePicker {
    ...
    public County getSelectedCounty() {
        if (getSelectedCity().getAreaList() != null &&
                getSelectedCity().getAreaList().size() > selectedThirdIndex) {
            return getSelectedCity().getAreaList().get(selectedThirdIndex);
        }
        return null;
    }
...
//                    }
            ...
            //修复二次进入地址选择，数据位置错乱的问题
            final boolean[] isFrist = {true};
            provinceView.setOnTouchListener(new View.OnTouchListener() {
                @Override
            ...
            countyView.setOnTouchListener(new View.OnTouchListener() {
                @Override
                public boolean onTouch(View v, MotionEvent event) {
                    isFrist[0] = false;
                    return false;
...
//                    }
    ...
    public static class AddressProvider implements DataProvider {
        ...
        private List<List<List<String>>> areaIdthirdList = new ArrayList<>();
        ...
        public List<String> provideThirdDataAreaId(int firstIndex, int secondIndex) {
            return areaIdthirdList.get(firstIndex).get(secondIndex);
        }
        ...
        private void parseData(List<Province> data) {
            ...
            for (int x = 0; x < provinceSize; x++) {
                ...
                areaIdfirstList.add(pro.getCode());
                List<City> cities = pro.getCityList();
                List<String> xCities = new ArrayList<>();
                List<String> xCitiesId = new ArrayList<>();
                List<List<String>> xCounties = new ArrayList<>();
                List<List<String>> xCountiesId = new ArrayList<>();
                int citySize = cities.size();
                //添加地市
                for (int y = 0; y < citySize; y++) {
                    City cit = cities.get(y);
                    cit.setProvinceId(pro.getCode());
                    xCities.add(cit.getName());
                    xCitiesId.add(cit.getCode());
                    List<County> counties = cit.getAreaList();
                    ArrayList<String> yCounties = new ArrayList<>();
                    ArrayList<String> yCountiesId = new ArrayList<>();
                    int countySize = counties.size();
                    //添加区县
                    if (countySize == 0) {
                        yCounties.add(cit.getName());
                        yCountiesId.add(cit.getCode());
                    } else {
                        for (int z = 0; z < countySize; z++) {
                            County cou = counties.get(z);
                            cou.setCityId(cit.getCode());
                            yCounties.add(cou.getName());
                            yCountiesId.add(cou.getCode());
                        }
                    }
                    xCounties.add(yCounties);
                    xCountiesId.add(yCountiesId);
                }
                secondList.add(xCities);
                thirdList.add(xCounties);
                areaIdsecondList.add(xCitiesId);
                areaIdthirdList.add(xCountiesId);
            }
        }
